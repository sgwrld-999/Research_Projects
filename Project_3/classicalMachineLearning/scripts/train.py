...
DATA_PATH = r"c:\Users\abhay\OneDrive\Desktop\SID\Research_Internship_Under_Dr_Rakesh_Matam\Project_3\dataset\conn1_encoded_normalised.csv"
...
def load_data():
    ...
    # Assuming 'label_stage_encoded' is the target and is the last column or named specifically
...
def split_data(X, y, train_size=20000):
    logging.info(f"Splitting data: {train_size} for training, rest for testing.")
    
    # Ensure we have enough data
    if len(X) <= train_size:
        raise ValueError(f"Dataset size ({len(X)}) is smaller than requested training size ({train_size})")
    
    # Stratified split to maintain class distribution in the small training set if possible
    # But user asked for specific 20k.
...
def plot_roc_curve(y_test, y_prob, model_name, save_dir, n_classes):
    # Get unique classes actually present in y_test
    present_classes = np.unique(y_test)
    
    # Binarize the output
    # Note: label_binarize will return columns for all classes in 'classes'
    # If a class is missing in y_test, its column will be all 0s
    ...
    for i in range(n_classes):
        # Check if class i is present in the data (either in test or was in training)
        # If the model predicts probabilities for it, we can plot it.
        # But if y_test_bin[:, i] is all 0 (class not in test set), roc_curve might warn or fail?
        # Actually roc_curve works with all 0s (returns nan or error depending on version).
...
def train_and_evaluate():
    X, y = load_data()
    X_train, X_test, y_train, y_test = split_data(X, y, train_size=20000)
    
    # Fixed number of classes for this project (0-4)
    n_classes = 5
    ...
    for name, model in models.items():
        try:
            ...
            if y_prob.shape[1] < n_classes:
                ...
                # Get classes known to the model
