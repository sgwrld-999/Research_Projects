...
LOGGER = get_logger('phase2_simplified')

def load_and_process_data(file_paths, label_col='Attack_label', dataset_type='edge-iiot'):
    dfs = []
    for path in file_paths:
        ...
        if dataset_type == 'ciciot':
            if 'label' in df.columns and 'Attack_label' not in df.columns:
                LOGGER.info("Creating binary Attack_label from 'label' column (CICIoT)")
                df['Attack_label'] = (df['label'] != 'BenignTraffic').astype(int)
                df.drop(columns=['label'], inplace=True)
        elif dataset_type == 'edge-iiot':
             # Edge-IIoT usually has Attack_label, but ensure it exists
             if 'Attack_label' not in df.columns and 'Attack_type' in df.columns:
                 # Fallback if Attack_label missing but type exists (unlikely for processed data)
                 pass
...
def main():
    parser = argparse.ArgumentParser(description='Train Simplified Phase 2 Model')
    parser.add_argument('--dataset', type=str, required=True, choices=['edge-iiot', 'ciciot'], help='Dataset name')
    ...
    if dataset_name == 'edge-iiot':
        ...
        train_files = [base_data_dir / "multiclass_train_80_20.csv"]
        test_files_paths = [
            base_data_dir / "multiclass_test_50_50.csv",
            base_data_dir / "multiclass_test_60_40.csv",
            base_data_dir / "multiclass_test_70_30.csv",
            base_data_dir / "multiclass_test_80_20.csv"
        ]
    ...
    run_id = f"{dataset_name}_phase_2_{timestamp}"
    LOGGER.info(f"Starting Simplified Training Run: {run_id}")
    
    # 1. Load Training Data
    LOGGER.info(f"Loading training data for {dataset_name}...")
    train_df, pipeline = load_and_process_data(train_files, label_col='Attack_label', dataset_type=dataset_name)
    le = pipeline['le']
    ...
    # 2. Train Simplified CNN-BiLSTM
    LOGGER.info("Training Simplified CNN-BiLSTM (64 Units)...")
    ...
    lstm_model, _ = train_phase2(
        ...
        epochs=20, # Reduced epochs for lighter run
        batch_size=256,
        class_weights=class_weights,
        lstm_units=64, # Reduced from 96
    ...
    for path in test_files_paths:
        ...
        if dataset_name == 'ciciot':
            if 'label' in test_df.columns:
                ...
                LOGGER.warning(f"Skipping {name}: Label column missing.")
                continue
        
        # Preprocess (Manual steps to avoid dropping label)
        df = test_df
        if label_col not in df.columns:
             LOGGER.warning(f"Skipping {name}: {label_col} missing.")
             continue
    ...
    LOGGER.info(f"Overall Results for {dataset_name}:")
