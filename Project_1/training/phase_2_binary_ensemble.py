...
LOGGER = get_logger('phase2_binary_ensemble')
...
def load_and_process_data(file_paths, label_col='Attack_label'):
    ...
    # Note: preprocess_multiclass_data handles label encoding. 
    # For binary 'Attack_label' (0/1), it might treat it as numeric or categorical.
    # We should ensure it's treated as a classification target.
    # If label_col is 'Attack_label', we need to make sure it's not dropped or scaled.
...
def main():
    ...
    run_id = datetime.now().strftime("binary_ensemble_%Y%m%d_%H%M%S")
    LOGGER.info(f"Starting Binary Ensemble Training Run: {run_id}")
    ...
    LOGGER.info("Loading and processing training data (Binary)...")
    # Use Attack_label as target
    ...
    LOGGER.info(f"Classes: {le.classes_}") # Should be [0, 1]
    ...
    seq_len = 10
    num_classes = len(le.classes_) # Should be 2
    ...
    for name, path in test_files:
        ...
        df = test_df
        # Filter if needed, but for binary we use all
        # Ensure label column is transformed
        ...
        try:
            auc = roc_auc_score(y_test_aligned, ensemble_probs[:, 1])
        except:
            auc = 0.0
            
        LOGGER.info(f"{name} Binary Ensemble Accuracy: {acc:.4f}, AUC: {auc:.4f}")
        
        overall_results[name] = {"accuracy": acc, "auc": auc}
        
        # Save Report
        target_names = [str(c) for c in le.classes_] # Should be ['0', '1'] or ['Normal', 'Attack'] depending on encoding
        report = classification_report(y_test_aligned, y_pred, target_names=target_names, output_dict=True)
        save_json(report, logs_dir / f"{run_id}_{name}_binary_report.json")
        
        # Plot CM
        cm = confusion_matrix(y_test_aligned, y_pred)
        plot_confusion_matrix(cm, target_names, out_path=plots_dir / f"{run_id}_{name}_binary_cm.png")
        
    LOGGER.info("Overall Binary Ensemble Results:")
    LOGGER.info(overall_results)
    save_json(overall_results, logs_dir / f"{run_id}_binary_results.json")
